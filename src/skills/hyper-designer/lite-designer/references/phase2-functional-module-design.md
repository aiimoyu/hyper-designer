...
目标值直接继承自需求分析说明书 §4 的技术约束，并细化为可测量指标。

#### 1.1 性能目标

| 指标 | 目标值 | 说明 |
|------|--------|------|
| [关键交互操作] 响应时间 | < [N]s | [触发条件] |
| [后台自动化任务] 完成时间 | < [N]s | [无用户介入] |
| [检索/查询操作] 响应时间 | < [N]ms | [并发条件] |
...
| 接口编号 | 接口名称 | 类型 | 提供方 | 消费方 | 说明 |
|----------|----------|------|--------|--------|------|
| IF-001 | [接口名] | [类型] | [模块A] | [模块B] | [用途描述] |
| IF-002 | [接口名] | [类型] | [模块B] | [模块C] | [用途描述] |
...
**2. 代码库分析**
- 使用 SearchCodebase/Grep 工具了解现有模块结构
- 识别核心模块和边界模块
- 确认哪些模块是稳定成熟的（不宜修改）
- 确认哪些模块是活跃开发的（可协商修改）

**3. 用户确认（必须）**
- 使用 AskUserQuestion 工具与用户确认围栏范围
- 特别关注模糊地带的归属
- 对于不确定的模块，宁可先问再定
...
❓ 需要您确认：
  - [模块E]：当前处于活跃开发状态，是否可以修改？
  - [模块F]：与本次需求相关但不在核心范围，是否纳入？
...
**SR-AR 草稿完成后，先展示结构给用户确认，再补充细节**：

```
以下是 SR-AR 分解草稿，请确认结构是否合理：
...
- [ ] 接口定义包含输入/输出/约束（不限 REST 形式）
- [ ] 修改围栏已参考历史文档和代码库分析
- [ ] 修改围栏已使用 AskUserQuestion 与用户确认
- [ ] 修改围栏明确列出允许和禁止的修改范围
- [ ] IR→SR→AR 追溯链完整，无断链
