...
**SE-ID映射 (必填)**:
每个SR的Who字段必须关联到具体的系统元素ID。

**Template (扩展)**:
```
Who: SE-XXX [系统元素名称] - [子系统/模块名称]
     负责团队: [可选：所属团队]
```

**Examples**:
- ✅ "SE-001 用户认证服务 (auth-service) - 认证子系统, Auth Team"
- ✅ "SE-003 订单处理模块 (order-module) - 订单子系统"
- ❌ "用户认证子系统" (缺少SE-ID)
- ❌ "SE-001" (缺少名称和职责描述)
...
**Who**: SE-001 用户认证服务 (auth-service) - 认证子系统 (Auth Team负责)
...
## DFX Summary Block (SR级DFX摘要)

每个SR在完成5W2H描述后，必须附加DFX摘要块，标注该SR的非功能性关注点。
...
| DFX类型 | 关注点 | 优先级 | 说明 |
|---------|--------|--------|------|
| 可靠性 | [需关注的故障场景] | 高/中/低 | [FMEA分析建议] |
| 安全性 | [威胁面/攻击向量] | 高/中/低 | [威胁建模建议] |
| 性能 | [关键路径/瓶颈点] | 高/中/低 | [性能预算建议] |
| 可维护性 | [可观测性需求] | 高/中/低 | [监控/日志要求] |
```

### DFX摘要示例

```markdown
#### DFX摘要 (SR-001: 第三方OAuth登录集成)

| DFX类型 | 关注点 | 优先级 | 说明 |
|---------|--------|--------|------|
| 可靠性 | OAuth Provider不可用时的降级处理 | 高 | 需FMEA分析: Provider超时、回调失败、token过期场景 |
| 安全性 | OAuth state参数防CSRF、token存储安全 | 高 | 需威胁建模: 重放攻击、token泄露、权限提升 |
| 性能 | OAuth流程端到端延迟(含Provider响应) | 中 | 性能预算: 总流程 < 3s, 本地处理 < 200ms |
| 可维护性 | OAuth流程日志和监控 | 中 | 需要: 登录成功率监控、Provider响应时间追踪、异常告警 |
```

### 5W2H Quick Checklist (扩展)

在完成SR的5W2H描述后，额外检查:

- [ ] **WHO包含SE-ID**: Who字段已关联到具体系统元素 (SE-XXX)
- [ ] **DFX摘要已填写**: 可靠性、安全性、性能、可维护性各项已标注
- [ ] **DFX优先级已设定**: 每个DFX关注点有明确优先级
- [ ] **DFX与系统规格一致**: DFX要求不与系统级规格矛盾
