...
| 变更类型 | 功能节点  | 变更点 | 对应需求 |
|----------|----------|------|----------|
| 🟢 新增 | 审批管理 | 新增审批流引擎 | SR-001 |
| 🟡 修改 | 订单提交 | 提交前增加审批校验 | SR-002 |
...
### 6.1 可用性 / 可靠性 (Availability & Reliability)

| 维度 | 涉及模块 | 措施 |
|------|----------|------|
| 容错与降级 | [模块] | [如：外部服务不可用时的降级行为 / 默认值兜底] |
| 重试与熔断 | [模块] | [如：指数退避重试 / 熔断阈值与恢复策略] |
| 超时控制 | [模块] | [如：各调用链环节超时上限] |
| 幂等设计 | [模块] | [如：写操作幂等 Key 方案] |
| 数据一致性 | [模块] | [如：最终一致 / 补偿事务 / 对账机制] |
| 可观测性 | [模块] | [如：关键路径日志 / Metrics / 链路追踪] |
| ... | ... | ... |
...
| 故障场景 | 影响范围 | 应对策略 | 恢复方式 |
|----------|----------|----------|----------|
| [如：下游服务宕机] | [如：功能 A 不可用] | [如：熔断 + 返回缓存/默认值] | [如：自动探活恢复] |
| [如：数据库主库故障] | [如：写操作不可用] | [如：只读降级] | [如：主从切换] |
| ... | ... | ... | ... |
