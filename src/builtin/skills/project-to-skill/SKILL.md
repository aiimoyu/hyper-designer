...
> **代码是唯一的事实来源。** README、文档、注释可能过时或错误，但代码永远反映真实状态。
>
> 当文档与代码冲突时，**以代码为准**。所有分析结论必须有代码证据支撑。
...
GitNexus 是本 skill 的**代码导航和索引工具**，用于快速定位代码节点、发现结构关系。

> **重要**：GitNexus 提供节点索引和关系，但细节很少。它能告诉你"代码在哪里"、"谁调用谁"，但不能告诉你"为什么这样设计"、"具体实现逻辑"。
>
> **正确用法**：GitNexus 定位 → Read 工具阅读实际代码 → 深入理解实现细节
>
> **错误用法**：仅凭 GitNexus 输出就下结论（会遗漏实现细节、设计意图）
