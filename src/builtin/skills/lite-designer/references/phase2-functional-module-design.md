...
**使用 Mermaid 图展示本次需求的修改边界，用不同颜色标注允许/禁止/条件修改的模块**：

```mermaid
graph TB
    subgraph 允许修改区域
        NewModule[新增模块A]
        ModifiedModule[修改模块B]
        NewFile1[新增文件/目录]
    end
    
    subgraph 条件修改区域
        ConditionalModule[条件修改模块C]
        ExtensionPoint[扩展点模块D]
    end
    ...
    NewModule --> ModifiedModule
    ModifiedModule --> ConditionalModule
    ConditionalModule -.->|需审批| CoreService
    
    style NewModule fill:#90EE90
    style ModifiedModule fill:#90EE90
    style NewFile1 fill:#90EE90
    style ConditionalModule fill:#FFD700
    style ExtensionPoint fill:#FFD700
    style CoreService fill:#FF6B6B
    style StableAPI fill:#FF6B6B
    style ThirdParty fill:#FF6B6B
    style LegacyModule fill:#FF6B6B
    style UnchangedA fill:#FFFFFF
    style UnchangedB fill:#FFFFFF
```

**围栏图例说明**：

- 🟢 绿色：允许修改 — 本次需求可直接修改的模块/文件
- 🟡 黄色：条件修改 — 需满足特定条件或审批后方可修改
- 🔴 红色：禁止修改 — 本需求范围内不得修改的模块/文件
- ⚪ 白色：无变更 — 本次需求不涉及的模块

**围栏详细说明表**：

| 围栏类型 | 模块/文件路径 | 说明 | 修改条件/原因 |
|----------|--------------|------|--------------|
| ✅ 允许修改 | [路径1] | [修改内容描述] | 本次需求核心范围 |
| ✅ 允许修改 | [路径2] | [修改内容描述] | 本次需求核心范围 |
| ⚠️ 条件修改 | [路径3] | [修改内容描述] | 需 [审批人/条件] 同意 |
| ⚠️ 条件修改 | [路径4] | [扩展点修改] | 仅限扩展点，不得修改核心逻辑 |
| ❌ 禁止修改 | [路径5] | 底层核心逻辑 | 影响系统稳定性，本需求范围外 |
| ❌ 禁止修改 | [路径6] | 稳定 API | 现有 API 兼容性，不得破坏 |
| ❌ 禁止修改 | [路径7] | 第三方依赖 | 由外部维护，不可直接修改 |

**围栏确认流程**：确定修改围栏前，需综合历史文档分析、代码库分析、用户确认，详见 §6 约束：修改围栏。
...
- [ ] 修改围栏图已绘制，用颜色标注允许/禁止/条件修改区域
- [ ] 架构变化说明表完整，列出所有变化类型

### SR-AR 分解

- [ ] 每个 SR 标注了对应的主要场景
- [ ] SR 总数合理（1-3 个），新增前已自问是否可合并
