...
1. **获取用户授权**
   - 调用 `HD_TOOL_ASK_USER`，消息：`下一步做什么？`，选项：`["进入下一阶段", "结束工作"]` 
   - 如果当前阶段为最后一个阶段时，只能选择结束工作。

2. **执行交接**
   - 若用户选择进入下一阶段，调用 `hd_handover` 并传入 `next_stage=` 参数，成功后输出：`本阶段任务结束后将自动进入下一阶段，再见。`
   - 若用户选择结束工作，调用 `hd_handover` 并传入参数 `end=true`，成功后输出：`工作流已结束，再见。`
